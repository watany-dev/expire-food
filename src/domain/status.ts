...
export type Status = "expired" | "past_best" | "warn" | "normal";

/**
 * 要件 4.2 の表示ルール。期限日 < 今日 は期限切れ、残り日数 < `warn_days` は期限間近。
 * 賞味期限切れは食べられることが多いので、消費期限切れ（expired）より弱い past_best にする（#41）
 */
export const itemStatus = (daysLeft: number, warnDays: number, kind: Item["kind"]): Status =>
  daysLeft < 0
    ? kind === "use_by"
      ? "expired"
      : "past_best"
