...
- `POST /api/extract` は multipart の `image`（JPEG / PNG / WebP、2MB 以下）を受ける。順序はレート制限（本文を読む前）→ 検証 → AI。レート制限超過は `429`、AI の失敗は `502`。どちらもクライアントは手入力へフォールバックする
  ...
  - 追記（#22）: 接続元ごとにも数え（`EXTRACT_IP_RATE_LIMITER`、1 分 30 回。IPv6 は /64 に丸める）、両方の枠があるときだけ通す。スペースは書き込みで無料で発行できるので、量産すれば space_id の枠は増やせるため。携帯回線の CGNAT などで同じ IP の後ろに他人がいるので、スペースごとの枠（10 回）より緩くする。space_id の枠で断ったリクエストは接続元の枠を減らさない
...
- ~~事前に作った複数のスペースの Cookie を使い回せばレート制限の枠は増やせる。無料プランの Workers AI は 1 日の上限で止まる（課金は増えない）ので、読み取りが使えなくなるだけとして許容する~~ → #22 で接続元ごとの枠を足した。多数の IP を使えば増やせる。Rate Limiting バインディングはロケーション単位・結果整合なので、上限は厳密ではない
