...
- [x] 撮影ボタンをフォーム先頭の大きなボタン「📷 撮影して読み取る」にする（ファイル入力は見た目だけ隠す。読み取り中は無効。#35）
- [x] 読み取り中に撮った写真の縮小画像（canvas）とスピナーを出す。読み終えたら消す。`prefers-reduced-motion` では回さない（#48）
- [x] 手入力でも登録済みの商品名（スペース内、重複なし）を `datalist` の候補に出す。タグと 1 回の往復で読む。読み取りの候補は同じ `datalist` の先頭に足す（#39）
- [x] 期限日のチップ「今日 / +3日 / +1週 / +1か月」（JST 基準、月末は次の月の末日に丸める。JS があるときだけ表示。#38）
...
- [x] Jev は「高補正モード」（フォームのチェックボックス。既定は off）のときだけ呼ぶ。`extract-eval` は判定なしと高補正モードを並べて出す（[ADR 0013](./adr/0013-judge-opt-in.md)）
- [ ] 実物パッケージ写真（牛乳・卵・パン・缶詰など）で精度を確認し、モデルとプロンプトを決める — Workers AI はリモート実行で Cloudflare の認証が要るため手作業。上のスクリプトで比べる。Jev の入出力の形（ADR 0007）と、高補正モードを既定にするか Jev を消すか（ADR 0013）もこのとき決める
...
- [x] 設定の共有URLの下に QR コード（`uqr` で行列にし、JSX の SVG `<path>` で描く。`raw()` は使わない。既定は `<details>` で畳む。#44）
...
- [x] manifest の `shortcuts` で、ホーム画面のアイコンの長押しから追加画面（`/items/new`）を開く（#40。Android Chrome）
