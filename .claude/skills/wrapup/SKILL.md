...
2. **正しさのレビュー**: Claude Code なら `/code-review`。それ以外のエージェント
   ...
   テスト欠落を指摘として列挙する。本アプリでは特に次を見る:
   - すべてのクエリに `space_id` 条件があるか（他スペースのデータを触れないか）
   - 日付は JST 基準か（UTC 15:00 前後の境界）
   - 入力はサーバー側でも Zod で検証しているか
...
6. **CI を通してコミット**。1〜5 で入った修正は実装と同じコミットでよいが、
   コメントのみの変更は別コミットにする。着手した Phase の `docs/ROADMAP.md`
   のチェックボックスも更新する
...
- 動作が変わる欠陥（誤動作・クラッシュ・境界条件・スペース越境・XSS / SQLi）
- 差分が短くなる指摘（`delete:` / `stdlib:` / `native:` / `yagni:` / `shrink:`）
- テストが無い分岐の追加

見送る:

- 好みの問題、命名の言い換え、差分の外
- 今の要件に無い一般化・将来への備え（`docs/requirements.md` の対象外を含む）
