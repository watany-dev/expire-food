...
対象は `src/`・`vite.config.ts`・`wrangler.jsonc`・`migrations/`・`.github/workflows/`。
`worker-configuration.d.ts`（生成物）と既存のマイグレーションファイル
（編集禁止）は触らない。`.semgrep/expire-food.tsx` の `ruleid:` / `ok:`
注釈はテストの一部なので消さない。

## 基準

- 削除: コードを読めば分かること。手順の説明、名前から自明な JSDoc、セクションバナー、テスト内の手順説明
- 残す: 理由。外部仕様の制約（Workers / D1 / Workers AI の制限、要件番号）、issue 参照、素直な方法を取らない理由、定数の根拠、fail-open / fail-close の方針、順序依存、テストの意図
- 混在していれば why の文だけ残す
...
既存 issue と `docs/ROADMAP.md` を探した上で無ければ issue を作り、コメントを issue 番号参照に書き換える。
